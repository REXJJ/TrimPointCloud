...
bool trimPointCloud(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
{
  ...
  Eigen::MatrixXd plane_yz=MatrixXd::Zero(1,4);
  Eigen::MatrixXd plane_xz=MatrixXd::Zero(1,4);
  plane_yz(0,0)=1.0;
  plane_xz(0,1)=1.0;
  Eigen::MatrixXd plane_ref=getPlane(points);
  double Ax=getAngle(plane_xz,plane_ref);
  double Ay=getAngle(plane_yz,plane_ref);
  dx=PI/2.0-Ax;
  dy=PI/2.0-Ay;
    cout << "Angle is "
    << dx <<" "<<dy << " degree"<<endl;
